...
public class Rods {
    ...
    public void solveTowersOfHanoi(int disksOnTop, int from, int spare, int destination) {
        // TODO
		// IMPORTANT:
		// always call moveDisk in the following manner:
		// moveDisk(from, dest); movesToSolve.addLast(from); movesToSolve.addLast(destination);
        // The two following statements record the movement.
        if(disksOnTop<=0)
        {return;}
        
        else
        {
             solveTowersOfHanoi(disksOnTop-1, from, destination, spare); //from to spare
             moveDisk(from, destination); movesToSolve.addLast(from); movesToSolve.addLast(destination);
             solveTowersOfHanoi(disksOnTop-1, spare, from, destination); //spare to destination
        }
    }
